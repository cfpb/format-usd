import { formatUSD } from '../index.js';
...
describe('handleStringInput', () => {
  it('Parse number strings with non-numeric characters', () => {
    expect(handleStringInput('9a99')).toBe(999);
    expect(handleStringInput('u123456')).toBe(123456);
    expect(handleStringInput('01234')).toBe(1234);
    expect(handleStringInput('$1,234,567')).toBe(1234567);
    expect(handleStringInput('Ilikethenumber5')).toBe(5);
    expect(handleStringInput('function somefunction() { do badstuff; }')).toBe(
      0
    );
  });

  it('Parse the first period as a decimal point', () => {
    expect(handleStringInput('4.22')).toBe(4.22);
    expect(handleStringInput('I.like.the.number.5')).toBe(0.5);
    expect(handleStringInput('1.2.3.4.5.6.7')).toBe(1.234567);
  });
});
